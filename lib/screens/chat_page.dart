...
import '../services/database_checker.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    // Check database setup in debug mode
    _checkDatabaseSetup();
  }

  Future<void> _checkDatabaseSetup() async {
    try {
      final results = await DatabaseChecker.checkMessagingSetup();
      if (results['errors'].isNotEmpty) {
        print('⚠️ Messaging setup issues detected:');
        for (final error in results['errors']) {
          print('  • $error');
        }
        print('💡 See MESSAGING_SETUP_INSTRUCTIONS.md for setup guide');
      }
    } catch (e) {
    ...
    _typingChan = Supabase.instance.client
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            ...
            if (data != null && data['user_id'] == widget.otherUserId) {
  ...
  void _openVideo(String url) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => FullscreenVideoPage(videoUrl: url)),
    );
  }
          ...
          children: [
            ...
            Column(
              ...
              children: [
                ...
                Text(
                  _otherTyping ? 'En train d’écrire...' : _lastSeen,
                  style: TextStyle(
                    color:
                        _otherTyping
                            ? Colors.orange
              ...
              builder: (ctx, snap) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final img =
                        c.contains('chat_media') &&
                        (c.endsWith('.jpg') || c.endsWith('.png'));
                          ...
                          if ((m['reactions'] as List).isNotEmpty)
                            Padding(
                              ...
                              child: Wrap(
                                spacing: 4,
                                children:
                                    (m['reactions'] as List).map<Widget>((r) {
                                      return Text(
                                        r['emoji'],
            ...
            child: Row(
              children: [
                IconButton(
                  ...
                  onPressed: () => _showEmojis(''),
                ),
                IconButton(
                  icon: const Icon(Icons.image),
                  onPressed: () => _pickAndSendMedia(video: false),
                ),
                IconButton(
                  icon: const Icon(Icons.videocam),
                  onPressed: () => _pickAndSendMedia(video: true),
                ),
                Expanded(
                  child: TextField(
